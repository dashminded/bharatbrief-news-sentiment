...
# ── Page setup 
st.set_page_config(page_title="BharatBrief", page_icon="ʙʜ", layout="wide")

EMOJI = {"Positive": "🟢", "Negative": "🔴", "Neutral":  "🟡"}

# ── Load data (FIXED CACHING: Expires every 10 minutes)
@st.cache_data(ttl=600)
def load_data():
    try:
        df = pd.read_csv("news_data.csv")
...
st.title(" BharatBrief")
st.markdown("<h3 style='color:#6b7280; margin-top:-10px;'> Track the Nation Through Data.</h3>", unsafe_allow_html=True)
st.divider()
...
# ── DYNAMIC METRICS HELPER FUNCTION ──
# This replaces your static numbers at the top
def display_metrics(filtered_df):
    if filtered_df.empty:
        st.info("No articles match this filter.")
        return
    
    total   = len(filtered_df)
    pos_pct = round((filtered_df["sentiment"] == "Positive").mean() * 100)
    neg_pct = round((filtered_df["sentiment"] == "Negative").mean() * 100)
    neu_pct = 100 - pos_pct - neg_pct
...
# ── 3 Tabs 
tab1, tab2, tab3 = st.tabs([" Morning Briefing — India", " Company News", " Tech & AI"])
...
with tab1:
    ...
    if not df_india.empty:
        sentiment_choice = st.selectbox("Filter by sentiment", ["All", "Positive", "Negative", "Neutral"], key="india_filter")
        
        if sentiment_choice != "All":
            df_india = df_india[df_india["sentiment"] == sentiment_choice]

        # Call the dynamic metrics function here
        display_metrics(df_india)

        for _, row in df_india.iterrows():
            emoji = EMOJI.get(row["sentiment"], "")
            time  = row["published_at"].strftime("%d %b, %I:%M %p")
            st.markdown(f"### {emoji} [{row['title']}]({row['url']})")
            st.caption(f" {row['source']}  ·   {time}  ·  Sentiment: **{row['sentiment']}**")
            st.divider()
...
with tab2:
    ...
    if not df_comp.empty:
        ...
        with col_a:
            ...
            company_choice = st.selectbox("Select Company", companies, key="comp_select")
        with col_b:
            comp_sent = st.selectbox("Filter by Sentiment", ["All", "Positive", "Negative", "Neutral"], key="comp_sent")
        ...
        # Call the dynamic metrics function here
        display_metrics(df_comp)

        for _, row in df_comp.iterrows():
            emoji = EMOJI.get(row["sentiment"], "")
            time  = row["published_at"].strftime("%d %b, %I:%M %p")
            st.markdown(f"### {emoji} [{row['title']}]({row['url']})")
            st.caption(f" **{row.get('company', '—')}** ·   {row['source']}  ·   {time}  ·  Sentiment: **{row['sentiment']}**")
            st.divider()
...
with tab3:
    ...
    if not df_tech.empty:
        col_x, col_y = st.columns(2)
        with col_x:
            tech_sent = st.selectbox("Filter by Sentiment", ["All", "Positive", "Negative", "Neutral"], key="tech_sent")
        with col_y:
            sources = ["All Sources"] + sorted(df_tech["source"].dropna().unique().tolist())
            source_choice = st.selectbox("Filter by Source", sources, key="tech_source")
